...
class CobrosSearchField extends ConsumerStatefulWidget {
  static const debounce = Duration(milliseconds: 350);

  const CobrosSearchField({super.key});

  @override
  ConsumerState<CobrosSearchField> createState() => _CobrosSearchFieldState();
}

class _CobrosSearchFieldState extends ConsumerState<CobrosSearchField> {
  ...
  void _onChanged(String value) {
    _debounce?.cancel();
    _debounce = Timer(CobrosSearchField.debounce, () {
      ref.read(cobrosSearchQueryProvider.notifier).onTextChanged(value);
    });
  }
  ...
  void _clear() {
    _debounce?.cancel();
    _controller.clear();
    ref.read(cobrosSearchQueryProvider.notifier).clear();
  }
  ...
  Widget build(BuildContext context) {
    ...
    final hasText = ref.watch(
      cobrosSearchQueryProvider.select((text) => text.isNotEmpty),
    );
